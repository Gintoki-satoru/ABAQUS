...
Fem_x = readmatrix('test.xlsx', 'Range', 'A:C');
Fem_y = readmatrix('test.xlsx', 'Range', 'E:G');
Fem_x_meri = readmatrix('Circumfrential.xlsx', 'Range', 'H:I');

figure;
plot(x, s_meri, 'b', 'LineWidth', 2,  'DisplayName','Analytical results'); hold on;
plot(Fem_x(:,1), Fem_x(:,2), 'ro', 'MarkerSize', 4, 'DisplayName','2D FEM results');
plot(Fem_y(:,1), Fem_y(:,2), 'LineWidth', 1.8, 'DisplayName','3D FEM results');
xlabel('x');
...
plot(Fem_x(:,1), Fem_x(:,3), 'ro', 'MarkerSize', 4, 'DisplayName','2D FEM results');
plot(Fem_y(:,1), Fem_y(:,3), 'LineWidth', 1.8, 'DisplayName','3D FEM results');
xlabel('y');
