...
r_inner = 50     # semi-axis in a
z_inner_up = 100   # semi-axis in c up
z_inner_dow = 200   # semi-axis in c down

n_up = 1        # egg exponent up
n_dow = 1.25        # egg exponent down

n_spline = 150  # number of spline points
N_theta = 20   # number of meridional regions

plyAngle = [60, -30, -60, 30, 60, -30, -60, 30]  # stacking sequence (degrees)
thick   = 0.261*plyAngle.__len__()  # total thickness
# thick = 2.372
N_part = plyAngle.__len__()  # number of partitions through thickness

r_outer = r_inner + thick
z_outer_up = z_inner_up + thick
z_outer_dow = z_inner_dow + thick

mesh_size = 0.5  # Mesh size

Press = 0.8 # Pressure load
compositeMaterialName = 'car_epx'  # 'cfk', 'AL', 'gfk', 'cfknew', 'car_epx', 'im7_epx'

# Strength parameters
Xt = 2382    # Longitudinal tensile strength
Yt = 30.8      # Transverse tensile strength
Xc = -669    # Longitudinal compressive strength
Yc = -162     # Transverse compressive strength
S = 59.7       # Shear strength
...
def superellipse_rz(theta, a, c, n):
    ...
    r = a * math.copysign(abs(cos_t) ** n, cos_t)
    z = c * math.copysign(abs(sin_t) ** n, sin_t)
    return r, z

inner_pts = []
outer_pts = []

theta_up = np.linspace(math.pi/2.0, 0, n_spline)

inner_up = []
outer_up = []

for th in theta_up:
    inner_up.append(superellipse_rz(th, r_inner, z_inner_up, n_up))
    outer_up.append(superellipse_rz(th, r_outer, z_outer_up, n_up))

theta_down = np.linspace(0.0, -math.pi/2.0, n_spline)

inner_down = []
outer_down = []

for th in theta_down:
    inner_down.append(superellipse_rz(th, r_inner, z_inner_dow, n_dow))
    outer_down.append(superellipse_rz(th, r_outer, z_outer_dow, n_dow))

inner_pts = inner_up + inner_down[1:]
outer_pts = outer_up + outer_down[1:]
...
theta_pick = math.radians(20.0)

theta_up = np.linspace(math.pi/2.0, 0, 250)
theta_down = np.linspace(0.0, -math.pi/2.0, 250)
...
for i in range(1, N_part):
    ...
    r_pick, z_pick = superellipse_rz(
        theta_pick,
        r_inner + 0.95*thick,
        z_inner_up + 0.95*thick,
        n_up
    )
    pickedFaces = f.findAt(((r_pick, z_pick, 0.0),))
    a_i = r_inner + i * thick / N_part
    c_i_up = z_inner_up + i * thick / N_part
    c_i_down = z_inner_dow + i * thick / N_part
    ...
    pts_up = [
        superellipse_rz(th, a_i, c_i_up, n_up)
        for th in theta_up
    ]
    pts_down = [
        superellipse_rz(th, a_i, c_i_down, n_dow)
        for th in theta_down
    ]
    part_pts = pts_up + pts_down[1:]
...
def surface_normal_rz(r, z, a, c, n, eps=1e-12):
    ...
    zc = max(abs(z) / c, eps)
    e = 2.0 / n
    dr = (e / a) * (ra ** (e - 1.0)) * math.copysign(1.0, r)
    dz = (e / c) * (zc ** (e - 1.0)) * math.copysign(1.0, z)
    mag = math.sqrt(dr*dr + dz*dz)
    return (dr / mag, dz / mag)

def get_params(theta):
    if theta >= 0.0:
        return r_inner, z_inner_up, n_up
    else:
        return r_inner, z_inner_dow, n_dow

delta = 2.0 * thick

theta_up = np.linspace(math.pi/2.0, 0, N_theta//2 + 1)
theta_down = np.linspace(0.0, -math.pi/2.0, N_theta//2 + 1)

theta_vals = list(theta_up) + list(theta_down[1:])
...
for i, theta in enumerate(theta_cut_vals):
    ...
    a, c, n = get_params(theta)
    r0, z0 = superellipse_rz(theta, a, c, n)
    nr, nz = surface_normal_rz(r0, z0, a, c, n)
...
# for k in range(N_part):
#     r_mid = 0.0
#     z_mid = z_inner_up + (k + 0.5) * thick / float(N_part)
...
N_sample = 80
theta_up = np.linspace(math.pi/2.0, 0, N_sample//2 + 1)
theta_down = np.linspace(0.0, -math.pi/2.0, N_sample//2 + 1)

theta_vals = list(theta_up[1:-1]) + list(theta_down[1:-1])
eps_theta = 1e-6
f1 = p.faces

def get_params(theta, r_mid):
    if theta >= 0.0:
        c_mid = z_inner_up + (r_mid - r_inner)
        return r_mid, c_mid, n_up
    else:
        c_mid = z_inner_dow + (r_mid - r_inner)
        return r_mid, c_mid, n_dow
...
################# Surface creation ###################
N_sample = 80
theta_up = np.linspace(math.pi/2.0, 0, N_sample//2 + 2)[1:-1]
theta_down = np.linspace(0.0, -math.pi/2.0, N_sample//2 + 2)[1:-1]

theta_vals = list(theta_up) + list(theta_down)

def get_outer_params(theta):
    if theta >= 0.0:
        return r_outer, z_outer_up, n_up
    else:
        return r_outer, z_outer_dow, n_dow
...
A_liner = superellipsoid_area(r_inner, r_inner, z_inner_up, n_up, 1)
A_ins = superellipsoid_area(r_inner + thick, r_inner + thick, z_inner_up + thick, n_up, 1)
A_outer = superellipsoid_area(r_inner + thick + t_ins, r_inner + thick + t_ins, z_inner_up + thick + t_ins, n_up, 1)#
A_outer_total = superellipsoid_area(r_inner + thick + t_ins + t_outer, r_inner + thick + t_ins + t_outer, z_inner_up + thick + t_ins + t_outer, n_up, 1)

# 2. Compute volume
V_inner = superellipsoid_volume(r_inner + thick, r_inner + thick, z_inner_up + thick, n_up, 1) - superellipsoid_volume(r_inner, r_inner, z_inner_up, n_up, 1)
V_ins = superellipsoid_volume(r_inner + thick + t_ins, r_inner + thick + t_ins, z_inner_up + thick + t_ins, n_up, 1) - superellipsoid_volume(r_inner + thick, r_inner + thick, z_inner_up + thick, n_up, 1)
V_outer = superellipsoid_volume(r_inner + thick + t_ins + t_outer, r_inner + thick + t_ins + t_outer, z_inner_up + thick + t_ins + t_outer, n_up, 1) - superellipsoid_volume(r_inner + thick + t_ins, r_inner + thick + t_ins, z_inner_up + thick + t_ins, n_up, 1)

# 3. Compute shape factors
S_liner = shape_factor(A_liner, V_inner, thick, r_inner, r_inner, z_inner_up)
S_ins   = shape_factor(A_ins,   V_ins, t_ins,   r_inner, r_inner, z_inner_up)
S_outer = shape_factor(A_outer, V_outer, t_outer, r_inner, r_inner, z_inner_up)
...
def solve_delta_for_point(r, z, a0, c0, thick, n_up, tol=1e-10, it=60):
    e=2/n_up
...
for elem in p.elements:
    ...
    c_ref = z_inner_up + delta_ref
    # --- normal + tangent ---
    n_r, n_z = surface_normal_rz(r_c, z_c, a_ref, c_ref, n_up)
    t_r, t_z = -n_z, n_r
    # --- local CSYS ---
...
def create_interface_node_set_by_delta_band(part, ply_k, N_part,
    ...
    for nlab in candidate_nodes:
        ...
        if d is None:
            d = solve_delta_for_point(r, z, r_inner, z_inner_up, thick, n_up)
            delta_cache[nlab] = d
        if d is None:
            n_none += 1
            continue
...
for k in range(N_part - 1):
    create_interface_node_set_by_delta_band(
        ...
        ply_k=k,
        N_part=N_part,
        r_inner=r_inner, z_inner_up=z_inner_up,
        thick=thick, n_up=n_up,
