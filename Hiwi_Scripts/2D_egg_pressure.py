...
r_inner = 540     # semi-axis in a
z_inner_up = 1980   # semi-axis in c up
z_inner_dow = 4920   # semi-axis in c down

# Upper part (z ≥ 0)
m_up_y = 2     # exponent in radial direction (r)
m_up_x = 2     # exponent in axial direction (z)

# Lower part (z < 0)
m_down_y = 1.6  # exponent in radial direction (r)
m_down_x = 2   # exponent in axial direction (z)

n_spline = 1000  # number of spline points
N_theta = 30   # number of meridional regions
...
compositeMaterialName = 'cfk'  # 'cfk', 'AL', 'car_epx', 'im7_epx'
...
USE_FIBER_CORR = False
nu12     = 0.34     # lamina major Poisson ratio
nu_f12   = 0.26    # fiber major Poisson ratio
E11      = 144990     # lamina E11
...
def superellipse_rz(theta, a, c, m_y, m_x):
    sin_t = math.sin(theta)
    # axial coordinate (exact)
    z = c * sin_t
    # radial coordinate from implicit equation
    term = 1.0 - abs(sin_t) ** m_x
    if term < 0.0:
        term = 0.0  # numerical safety
    r = a * (term ** (1.0 / m_y))
    return r, z
...
for th in theta_up:
    inner_up.append(superellipse_rz(th, r_inner, z_inner_up, m_up_y, m_up_x))
    outer_up.append(superellipse_rz(th, r_outer, z_outer_up, m_up_y, m_up_x))
...
for th in theta_down:
    inner_down.append(superellipse_rz(th, r_inner, z_inner_dow, m_down_y, m_down_x))
    outer_down.append(superellipse_rz(th, r_outer, z_outer_dow, m_down_y, m_down_x))
...
theta_up = np.linspace(math.pi/2.0, 0, 500)
theta_down = np.linspace(0.0, -math.pi/2.0, 500)
...
for i in range(1, N_part):
    ...
    pts_up = [
    superellipse_rz(th, a_i, c_i_up, m_up_y, m_up_x)
    for th in theta_up
    ]
    pts_down = [
    superellipse_rz(th, a_i, c_i_down, m_down_y, m_down_x)
    for th in theta_down
    ]
...
def surface_normal_rz(r, z, a, c, m_y, m_x, eps=1e-12):
    ra = max(abs(r) / a, eps)
    zc = max(abs(z) / c, eps)
    dr = (m_y / a) * (ra ** (m_y - 1.0)) * math.copysign(1.0, r)
    dz = (m_x / c) * (zc ** (m_x - 1.0)) * math.copysign(1.0, z)
    mag = math.sqrt(dr*dr + dz*dz)
    return (dr / mag, dz / mag)

def get_params(theta):
    if theta >= 0.0:
        return r_inner, z_inner_up, m_up_y, m_up_x
    else:
        return r_inner, z_inner_dow, m_down_y, m_down_x
...
# for i, theta in enumerate(theta_cut_vals[1:], start=1):
    # --- point on mid-thickness surface ---
    a, c, m_y, m_x = get_params(theta)
    r0, z0 = superellipse_rz(theta, a, c, m_y, m_x)
    nr, nz = surface_normal_rz(r0, z0, a, c, m_y, m_x)
...
def get_params(theta, r_mid):
    if theta >= 0.0:
        ...
        return r_mid, c_mid, m_up_y, m_up_x
    else:
        c_mid = z_inner_dow + (r_mid - r_inner)
        return r_mid, c_mid, m_down_y, m_down_x
...
for k in range(N_part):
    ...
    for theta in theta_vals:
        if abs(theta) < eps_theta:
            continue
        a, c, m_y, m_x = get_params(theta, r_mid)
        r0, z0 = superellipse_rz(theta, a, c, m_y, m_x)
        pt = (r0, z0, 0.0)
        try:
...
for theta in theta_vals:
    ...
    a, c, m_y, m_x = get_inner_params(theta)
    r0, z0 = superellipse_rz(theta, a, c, m_y, m_x)
    pt = (r0, z0, 0.0)
    found = s2.getClosest(
        coordinates=(pt,),
...
p.seedPart(size=mesh_size, deviationFactor=0.01, minSizeFactor=0.8)
...
def F_val(r, z, a, c, m_y, m_x):
    return (abs(r)/a)**m_y + (abs(z)/c)**m_x - 1.0


def find_layer_from_surfaces(r_c, z_c, a0, c0, m_y, m_x, thick, N_part):
    for k in range(N_part):
        a_k = a0 + (k+1) * thick / N_part
        c_k = c0 + (k+1) * thick / N_part
        if F_val(r_c, z_c, a_k, c_k, m_y, m_x) <= 0:
            return k
    return None
...
# myModel = mdb.models['EllipseModel_2D']
# p = myModel.parts['SuperEllipsoid_2D']
...
for elem in p.elements:
    ...
    a0, c0, m_y, m_x = get_geom_params(z_c)
    k = find_layer_from_surfaces(r_c, z_c, a0, c0, m_y, m_x, thick, N_part)
    if k is None:
        continue
    ...
    a_ref = a0 + delta_ref
    c_ref = c0 + delta_ref
    # correct normal (from surface point, not centroid!)
    n_r, n_z = surface_normal_rz(r_c, z_c, a_ref, c_ref, m_y, m_x)
    t_r, t_z = n_z, -n_r
    # --- CSYS ---
    dcsys = p.DatumCsysByThreePoints(
        ...
        origin=(r_c, z_c, 0.0),
        point1=(r_c + t_r, z_c + t_z, 0.0),
        point2=(r_c + n_r, z_c + n_z, 0.0)
    )
...
job = mdb.Job(name='Job-1', model='EllipseModel_2D', description='', type=ANALYSIS, 
        ...
        scratch='D:\\abaqus_tmp', resultsFormat=ODB, numThreadsPerMpiProcess=0, numCpus=6, 
