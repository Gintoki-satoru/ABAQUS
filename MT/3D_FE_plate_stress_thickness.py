...
def importStressInner2DFiniteElement(directory, N):
    stressComponent = [1, 2, 3, 4,5,6]  # Stress components to be processed
    sumStresses = []
    ...
    for ii in range(N):
        ...
        circPos = ['TG','TS']  # Circumferential positions to consider
...
def saveToCSV(directory, save_dir, N):
    ylabelNorm = [r'$\sigma_{xx}$ in MPa', r'${\sigma}_{yy}$ in MPa', r'${\sigma}_{zz}$ in MPa', r'${\tau}_{yz}$ in MPa',r'${\tau}_{xz}$ in MPa',r'${\tau}_{xy}$ in MPa']
    outputStressComp = ['Sigxx', 'Sigyy', 'Sigzz', 'Tauyz','Tauxz','Tauxy']
