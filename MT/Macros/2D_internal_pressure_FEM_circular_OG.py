...
modelName = 'internal_pressure_' + str(ric_plyAngle) + '_mR' + str(mR) +'_mT'+ str(mT) + '_'+ analysis_currentDateTime
...
def sketchLModel():
	...
	LModelSketch.ArcByCenterEnds(
		...
		point2=(ct.pol2cart_x(rk[0], thetaEdges[1]), ct.pol2cart_y(rk[0], thetaEdges[1])), #thetaEdges-1
	...
	for ii in range(N - 1):
		LModelPartitionSketch.offset(
			...
			objectList=(
				...
				g.findAt((ct.pol2cart_x(rk[0], thetaEdges[0]), 0 - ltan / 2))), side=RIGHT) #g.findAt((ct.pol2cart_x(rk[0], thetaEdges[-1]) - ltan / 2, ct.pol2cart_y(rk[0], thetaEdges[-1])))
...
def meshLPart(mR, mT, mFlanken, mBias, mRRatio, mTRatio):
	...
	for ii in range(len(rk)):
		  ...
		  LModelPart.seedEdgeByBias(
			biasMethod=DOUBLE, #SINGLE
			endEdges=curvedCompositeMeshEdge, #endEdges
			ratio=mTRatio,
			number=mT/2,
			constraint=FINER
			) 
...
def boundaryConditionSurfaceLoad():
	for jj in range(1,3):#enumerate([thetaEdges[0],thetaEdges[-1]]):
		curvedCompositeStringFixCircFaces = ''
		for ii in range(len(rFacesAll)):
			curvedCompositeStringFixCircFaces = curvedCompositeStringFixCircFaces + str(((ct.pol2cart_x(rFacesAll[ii], thetaEdges[jj-1]), ct.pol2cart_y(rFacesAll[ii], thetaEdges[jj-1]) + (jj-2)*ltan, 0.0),),) + ','#curvedCompositeStringFixCircFaces = curvedCompositeStringFixCircFaces + str(((ct.pol2cart_x(rFacesAll[ii], ll), ct.pol2cart_y(rFacesAll[ii], ll), 0.0),),) + ','
			curvedCompositeFixCircFacesExec = 'curvedCompositeFixCircFaces = LModelInstance.edges.findAt(' + curvedCompositeStringFixCircFaces + ')'
			exec(curvedCompositeFixCircFacesExec)
		...
		if jj == 2:
			...
			LModel.DisplacementBC(name='Fix_U_SS_'+ str(jj), createStepName=stepPrevious, region=curvedCompositeFixCircSet, u2=0.0)
...
#		exec(curvedCompositeStringFacesRightExec)
		...
		right_pts = [ ((ct.pol2cart_x(rk[0], angleEval_FEInnner-1e-5),
					ct.pol2cart_y(rk[0], angleEval_FEInnner-1e-5),
					0.0),)
					for _ in range(int(len(thetaPartitionFaces)/2)) ]
		left_pts  = [ ((ct.pol2cart_x(rk[0], angleEval -1e-5),
					ct.pol2cart_y(rk[0], angleEval -1e-5),
					0.0),)
					for _ in range(int(len(thetaPartitionFaces)/2)) ]
		# 2) concatenate
		all_pts = right_pts + left_pts
		# 3) hand them off directly
		curvedCompositeInnerPressureFaces = LModelInstance.edges.findAt(*all_pts)
		if boolCylindricalPressure:
