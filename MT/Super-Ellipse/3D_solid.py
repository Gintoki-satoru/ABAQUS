...
def create_layer_part(model, layer_index):
    ...
    # --- Shift last point ---
    def shift_last_point(curve_points, theta, a, b, c, phi_vals, n1, n2, shift_factor):
        phi_last = phi_vals[-1]
        phi_shifted = phi_last - shift_factor
        return superellipsoid_point_3d(phi_shifted, theta, a, b, c, n1, n2)
    n = max(n1, n2)
    shift_deg = 20 * math.exp(-2.7 * n**1.1)
    shift = math.radians(shift_deg)
    outer_case2[-1] = shift_last_point(outer_case2, 0.0, a_o, b_o, c_o, phi_vals, n1, n2, shift)
    inner_case2[-1] = shift_last_point(inner_case2, 0.0, a_i, b_i, c_i, phi_vals, n1, n2, shift)
    outer_case3[-1] = shift_last_point(outer_case3, theta_90, a_o, b_o, c_o, phi_vals, n1, n2, shift)
    inner_case3[-1] = shift_last_point(inner_case3, theta_90, a_i, b_i, c_i, phi_vals, n1, n2, shift)
    outer_case4[-1] = shift_last_point(outer_case4, theta_45, a_o, b_o, c_o, phi_vals, n1, n2, shift)
    inner_case4[-1] = shift_last_point(inner_case4, theta_45, a_i, b_i, c_i, phi_vals, n1, n2, shift)
    # --- Create part and datum points/wires ---
